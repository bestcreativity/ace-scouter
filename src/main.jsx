...
const navItems = [
  ...
  { label: 'Pitch queue', icon: Sparkles },
...
function Dashboard({ session }) {
  const [activeNav, setActiveNav] = useState('Search Leads')
  ...
  const updateLeadEmail = async (leadId, email) => {
    const { error } = await supabase.from('leads').update({ decision_maker_email: email, email_verified: true }).eq('id', leadId)
    if (error) throw error
    await loadData()
  }

  const sendDraft = async (draftId) => {
    const { error } = await supabase.functions.invoke('send-approved-draft', { body: { draftId } })
    if (error) throw error
    await loadData()
  }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        </> : activeNav === 'Search Leads' ? <SearchLeads campaigns={campaigns} leads={leads} loading={dataLoading} onSearch={async (campaign) => { try { const savedCampaign = await createCampaign(campaign); notify('Search saved. Finding businesses...'); const count = await scoutCampaign(savedCampaign); notify(count ? `${count} businesses found.` : 'No named businesses found for that search.') } catch (error) { notify(`Search failed: ${error.message}`) } }} /> : activeNav === 'Pitch queue' ? <PitchQueue drafts={drafts} emailLogs={emailLogs} onGenerate={async () => { try { const count = await generateDrafts(); notify(count ? `${count} pitch drafts created.` : 'No new leads need drafts.') } catch (error) { notify(`Could not create drafts: ${error.message}`) } }} onUpdate={async (id, status) => { try { await updateDraft(id, status); notify(`Draft ${status}.`) } catch (error) { notify(`Could not update draft: ${error.message}`) } }} onRecipientEmail={async (leadId, email) => { try { await updateLeadEmail(leadId, email); notify('Recipient email saved.') } catch (error) { notify(`Could not save recipient: ${error.message}`) } }} onSend={async (id) => { try { await sendDraft(id); notify('Email sent.') } catch (error) { notify(`Could not send email: ${error.message}`) } }} /> : activeNav === 'Integrations' ? <SenderSettings email={sendingEmail} onSave={async (email) => { try { await updateSendingEmail(email); notify('Sending email updated.') } catch (error) { notify(`Could not update sending email: ${error.message}`) } }} /> : <PlaceholderView activeNav={activeNav} onPrimary={() => setShowCampaign(true)} />}
      </main>
...
function SearchLeads({ campaigns, leads, loading, onSearch }) {
  const [niche, setNiche] = useState('')
  const [location, setLocation] = useState('')
  const [limit, setLimit] = useState(10)
  const [searching, setSearching] = useState(false)
  const submit = async (event) => { event.preventDefault(); setSearching(true); await onSearch({ target_niche: niche, location, daily_limit: Number(limit), status: 'active' }); setSearching(false) }
  return <section className="module-view search-module"><div className="eyebrow"><Search size={13} /> Prospect discovery</div><div className="module-heading"><div><h1>Search Leads</h1><p>Describe the businesses you want to find. ACE searches free public business data.</p></div></div><form className="panel search-form" onSubmit={submit}><div className="search-form-grid"><label>What are you looking for?<input required value={niche} onChange={(event) => setNiche(event.target.value)} placeholder="Dental clinics, HVAC contractors..." /></label><label>Where?<input required value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Austin, TX" /></label><label>Maximum leads<select value={limit} onChange={(event) => setLimit(event.target.value)}><option value="5">5 leads</option><option value="10">10 leads</option></select></label></div><div className="search-form-footer"><span><Globe2 size={14} /> OpenStreetMap + public business data</span><button className="primary-button" disabled={searching}>{searching ? 'Searching...' : 'Search businesses'} <Search size={15} /></button></div></form><div className="search-results"><div className="results-heading"><div><h2>Your workspace</h2><p>{loading ? 'Loading live records...' : `${campaigns.length} searches · ${leads.length} leads found`}</p></div></div>{leads.length === 0 && !loading ? <div className="panel empty-module"><Target size={20} /><strong>No leads found yet</strong><span>Run your first search to find real businesses.</span></div> : <div className="panel results-list">{leads.slice(0, 10).map((lead) => <div className="result-row" key={lead.id}><div className="company-avatar sky">{lead.business_name.slice(0, 2).toUpperCase()}</div><div><strong>{lead.business_name}</strong><span>{lead.campaigns?.location || 'Location unavailable'} · {lead.website || 'No website listed'}</span></div><small>{lead.decision_maker_email || 'Email needed in Pitch queue'}</small></div>)}</div>}</div></section>
}

function PitchQueue({ drafts, emailLogs, onGenerate, onUpdate, onRecipientEmail, onSend }) {
  return <section className="module-view"><div className="eyebrow"><Sparkles size={13} /> Human review</div><div className="module-heading"><div><h1>Pitch queue</h1><p>Review each personalized message, add the recipient, then send manually.</p></div><button className="primary-button" onClick={onGenerate}><Sparkles size={15} /> Generate drafts</button></div>{drafts.length === 0 ? <div className="panel empty-module"><Sparkles size={20} /><strong>No drafts waiting</strong><span>Search for leads first, then generate personalized drafts here.</span></div> : <div className="draft-list">{drafts.map((draft) => { const sentLog = emailLogs.find((log) => log.lead_id === draft.lead_id); return <article className="panel draft-card" key={draft.id}><div className="draft-meta"><span>{draft.leads?.business_name || 'Unknown business'}</span><small>{draft.leads?.decision_maker_email || 'Recipient email needed'}</small></div><label className="recipient-field">Recipient email<input type="email" defaultValue={draft.leads?.decision_maker_email || ''} placeholder="owner@business.com" onBlur={(event) => event.target.value && event.target.value !== draft.leads?.decision_maker_email && onRecipientEmail(draft.leads.id, event.target.value)} /></label><h2>{draft.subject}</h2><p>{draft.body}</p><div className="draft-footer"><span className={`draft-status ${draft.status}`}>{draft.status}</span><span className="delivery-status">{sentLog ? sentLog.opened ? 'Opened' : sentLog.sent_at ? 'Sent · not opened' : 'Queued' : 'Not sent'}</span>{draft.status === 'pending' && <div><button className="outline-button" onClick={() => onUpdate(draft.id, 'rejected')}>Reject</button><button className="primary-button" onClick={() => onUpdate(draft.id, 'approved')}><Check size={14} /> Approve</button></div>}{draft.status === 'approved' && <button className="primary-button" disabled={!draft.leads?.decision_maker_email} onClick={() => onSend(draft.id)}><Send size={14} /> Send email</button>}</div></article> })}</div>}</section>
}
